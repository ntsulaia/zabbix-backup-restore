...
def zabbix_server_stop():
    cmd = "/usr/bin/systemctl stop zabbix-server.service"
...
def zabbix_server_start():
    cmd = "/usr/bin/systemctl start zabbix-server.service"
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
...
def zabbix_database_backup(backup_path):
    cmd = "/usr/bin/mysqldump --all-databases > {0}/dump.sql"
...
def zabbix_httpd_backup(backup_path):
    cmd = "/usr/bin/cp /etc/httpd/conf.d/zabbix.conf {0}"
